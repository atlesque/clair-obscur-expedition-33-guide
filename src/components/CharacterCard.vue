...
<template>
  <article class="card character">
    <div class="card-head"><div><p class="eyebrow">TRACKED CHARACTER / {{ character.id }}</p><h2>{{ character.name }}</h2></div><div class="card-head-actions"><span class="level">LV {{ character.level }}</span><div class="character-menu"><button type="button" class="icon-button more-button" aria-label="More character actions" :aria-expanded="actionsOpen" @click="actionsOpen = !actionsOpen">⋯</button><div v-if="actionsOpen" class="character-popover" role="menu"><button type="button" role="menuitem" class="quiet" @click="removeCharacter">Remove character</button></div></div></div></div>
