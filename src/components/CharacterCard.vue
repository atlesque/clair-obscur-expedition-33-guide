  ...
  <article class="card character">
    ...
    <div class="stats">
      <div v-for="a in ATTRIBUTES" :key="a">
        ...
        ><b>{{ character.invested[a] }}</b>
      </div>
    </div>
    <p v-if="character.pending" class="advice">
      <strong>Pending recommendation</strong><br /><span
        v-for="(amount, a) in character.pending.spend"
        :key="a"
        >+{{ amount }} {{ attributeLabels[a] }} </span
      ><br /><small>{{ character.pending.explanation }}</small>
    </p>
    <div class="actions">
      <button @click="$emit('update', character)">Update progress</button
      ><button @click="$emit('advise', character)">
        Get advice <span class="sr-only">for {{ character.name }}</span></button
      ><button
        v-if="character.pending"
        class="primary"
        @click="$emit('confirm', character)"
      >
        I've applied these
      </button>
      <button class="quiet" @click="$emit('remove', character)">Remove</button>
    </div>
