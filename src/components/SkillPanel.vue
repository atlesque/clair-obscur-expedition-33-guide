...
const open=ref(false); const scaling=ref<Record<string,string>>({...props.character.weaponScaling}); const manual=ref((props.character.unlockedSkills??[]).join(', '));
function save(){const c={...props.character,weaponScaling:Object.fromEntries(Object.entries(scaling.value).filter(([,v])=>v)),skillSetupComplete:true,unlockedSkills:props.character.skillAcquisition==='learned'?manual.value.split(',').map(x=>x.trim()).filter(Boolean):[...(props.character.unlockedSkills??[])],skillPoints:props.character.skillPoints??0};emit('update',c);open.value=false;}
