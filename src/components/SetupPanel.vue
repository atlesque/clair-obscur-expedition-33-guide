  ...
  <section class="setup-panel">
    ...
    <div class="setup-form">
      ...
      <div v-if="midway" v-for="key in selected" :key="key" class="midway-fields">
        ...
        <label>Current level<input type="number" min="1" max="99" v-model.number="drafts[key]!.level" @focus="toggle(key)" /></label>
        <label>Available attribute points<input type="number" min="0" v-model.number="drafts[key]!.points" @focus="toggle(key)" /></label>
        <div v-for="a in ['vitality','might','agility','defence','luck']" :key="a"><label>{{ a }}<input type="number" min="0" max="99" v-model.number="drafts[key]!.invested[a as keyof Attributes]" @focus="toggle(key)" /></label></div>
      </div>
